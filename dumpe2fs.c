...
{
	fprintf(stderr, _("Usage: %s [-bfghimxV] wrong [-o superblock=<num>] "
...
static void print_inodes(unsigned long group,char *bitmap,
			unsigned long num,unsigned long offset,int ratio)
{
...
static void list_desc(ext2_filsys fs, int grp_only)
{
	...
	for (i = 0; i < fs->group_desc_count; i++) {
		...
		if (inode_bitmap) {
			...
			fputs(" Here are the inode numbers ",stdout);
			if (retval)
				com_err("list_desc", retval,
...
//volatile unsigned int *p=(volatile unsigned int *)0x32769;
//printf("%u/n",*p);

printf("%d/n blocksize is  -->>/n",fs->blocksize/1024);
#ifdef ENABLE_NLS
