...
2) Setting the flags like Journalling and software support 
3) Opening file system using ext2fs_open
4) Printing the journal information 
5) Printing the journal inline information
5) Reading the bitmaps
6) List description
...
static void list_desc (ext2_filsys fs)
{
	unsigned long i;
	long diff;
	blk_t	first_block, last_block;                                                   // typedef __u32		blk_t;
	blk_t	super_blk, old_desc_blk, new_desc_blk;                                     // typedef __u32		blk_t;
	char *block_bitmap=NULL, *inode_bitmap=NULL;
	int inode_blocks_per_group, old_desc_blocks, reserved_gdt;
	int has_super;

	if (fs->block_map)
		block_bitmap = fs->block_map->bitmap;				          // Print block bitmap
	if (fs->inode_map)
		inode_bitmap = fs->inode_map->bitmap;                                     // Print i-node bitmap

	inode_blocks_per_group = ((fs->super->s_inodes_per_group *                        // Calculating I-node blocks for group
				   EXT2_INODE_SIZE(fs->super)) +
				  EXT2_BLOCK_SIZE(fs->super) - 1) /
				 EXT2_BLOCK_SIZE(fs->super);
	
	
	reserved_gdt = fs->super->s_reserved_gdt_blocks;					// What is this GDT for?
	fputc('\n', stdout);
	first_block = fs->super->s_first_data_block;                                      // First Block
	...
	for (i = 0; i < fs->group_desc_count; i++) {                                      // For loop to print info on all the groups
		first_block = ext2fs_group_first_block(fs, i);                            // Calculating first blcok for group i
		last_block = ext2fs_group_last_blo ck(fs, i);                             // Calculating last block for group i

		ext2fs_super_and_bgd_loc(fs, i, &super_blk,&old_desc_blk, &new_desc_blk, 0); // What does this function do?
		...
		if (has_super) {                                                            // Printing super block if any
			printf (_("  %s superblock at "),
				i == 0 ? _("Primary") : _("Backup"));
			print_number(super_blk);
		}
		if (old_desc_blk) {                                                        //  Print group descriptor and reserved GDT blocks
		...
		print_number(fs->group_desc[i].bg_block_bitmap);                           // Blcok Bitmap
		diff = fs->group_desc[i].bg_block_bitmap - first_block;                    
		if (diff >= 0)
			printf(" (+%ld)", diff);
		fputs(_(", Inode bitmap at "), stdout);
		print_number(fs->group_desc[i].bg_inode_bitmap);                           // I-node bitmap
		diff = fs->group_desc[i].bg_inode_bitmap - first_block;
		if (diff >= 0)
			printf(" (+%ld)", diff);
		fputs(_("\n  Inode table at "), stdout);
		print_range(fs->group_desc[i].bg_inode_table, fs->group_desc[i].bg_inode_table + inode_blocks_per_group - 1);    // I node table calculation at = group_desc[i].bg_inode_table- (group_desc[i].bg_inode_table_ no. of inodes for each group -1) 
		diff = fs->group_desc[i].bg_inode_table - first_block;
 

/*Whenever a user or a program needs access to a file, the operating system first searches for the exact and unique inode (inode number),
 in a table called as an inode table. In fact the program or the user who needs access to a file,
 reaches the file with the help of the inode number found from the inode table. */
		...
		if (diff > 0)
			printf(" (+%ld)", diff);
		printf (_("\n  %d free blocks, %d free inodes, "                            // Printing Free blcok i-nodes
			  "%d directories\n"),
			fs->group_desc[i].bg_free_blocks_count,
			fs->group_desc[i].bg_free_inodes_count,
			fs->group_desc[i].bg_used_dirs_count);
		if (block_bitmap) {
			fputs(_("  Free blocks: "), stdout);                                 // Free blocks
...
/* Reads the i-node information from the file system using function ext2fs_read_inode() and prints the 
journal size */
...
int main (int argc, char ** argv)
{
	...
	while ((c = getopt (argc, argv, "bfhixVo:")) != EOF) {   // getopt() parses the command line arguments. An element of argv that starts with '-', is an option element
		switch (c) {                                     // The characters of this element (aside from the initial '-')are option characters.If getopt() is called repeatedly, it returns
	...
	if (force)
		flags |= EXT2_FLAG_FORCE; 							//Open the filesystem regardless of the feature sets listed in the superblock.
	if (image_dump)
	...
	if (use_superblock && !use_blocksize) {
		for (use_blocksize = EXT2_MIN_BLOCK_SIZE;
		     ...
		     use_blocksize *= 2) {
			retval = ext2fs_open (device_name, flags,                       // Opens a file system fs, using manager. Superblock number should be used when opening and use_blocksize 
					      use_superblock,				// specifies block size used by the file system. Source: http://www.fifi.org/doc/e2fslibs/html-info/libext2fs_2.html
